...
export class AllPoints {
     ...
     */
    calculateNewPolygonPointsWithInnerPoints(points, innerPoints, inverse = false) {
        if (inverse) {
        ...
        const processRestOfPoints = (innerPointIndex) => {
            for (let i = 1; i < points.length; i++) {
                polygonPoints.push(points[i]);
            }

            let lastPointPassed = false;
     ...
     * @param {ExtPoint[]} innerPoints
     */
    updateNewInnerPoints(newPolygonPoints) {
        const polygonLines = GeomUtils.getLinesFromPolygonPoints(newPolygonPoints);
        const innerLines = GeomUtils.getLinesFromPolygonPoints(this.innerPolygonPointsClockwise);
        ...
        const nextIndex = (array, index, increment = 1) => {
